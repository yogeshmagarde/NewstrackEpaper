...
def extract_epaperdata():
    ...
    save_path = os.path.join('C:\\FLASK2\\pdf2', pdf_file.filename)
    pdf_file.save(save_path)

    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"cropped_image_{current_time}.jpg"
    image_path = os.path.join('C:\\FLASK2\\static', filename)
    ...
    if len(image_filenames) >MAX_IMAGES:
        ...
        os.remove(os.path.join('C:\\FLASK2\\static', oldest_image))
